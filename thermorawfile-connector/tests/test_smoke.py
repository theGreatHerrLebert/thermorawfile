...
def test_open_and_metadata():
    with thermorawfile.RawFile(FIXTURE) as rf:  # context manager (wrapper)
        ...
        assert list(rf) == list(range(1, 96))  # __iter__ yields scan numbers


def test_peaks_are_numpy():
    rf = thermorawfile.RawFile(FIXTURE)
    p = rf.peaks(2)
    mz, intensity = p  # Peaks unpacks as a tuple...
    assert p.mz is mz and p.intensity is intensity  # ...and exposes named fields
...
def test_filter_event_and_params():
    ...
    ev = rf.scan_event(2)
    assert ev.ms_order == 2  # ScanEvent dataclass (wrapper)
